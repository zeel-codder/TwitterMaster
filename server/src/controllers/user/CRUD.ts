...
const GetTokeInfo = async (req: Request, res: Response) => {
  try {
    const { token } = req.body;
    let user = await jwt.verify(token, process.env.JWT_SECRET || "");

    return res.status(200).send(ResultLoader("User Info", { user: user }));
  } catch (e: any) {
    res.status(404).send(ErrorLoader("Token Invalid", e.message));
  }
};
...
const GetUsers = async (req: Request, res: Response) => {
  try {
    ...
    if (List.length < number) {
      return res
        .status(200)
        .send(ResultLoader("All Tweet", { List: UserList, isEnd: true }));
    }

    res
      .status(200)
      .send(ResultLoader("All Users", { List: UserList, isEnd: false }));
  } catch (e: any) {
...
const AddUser = async (req: Request, res: Response) => {
  try {
    ...
    if (findByName || findByEmail) {
      ...
      return res
        .status(500)
        .send(ErrorLoader("User Name and Email Exits", "UserFound"));
    }
...
const SingIn = async (req: Request, res: Response) => {
  try {
    ...
    if (!findByName) {
      ...
      return res
        .status(500)
        .send(ErrorLoader("User Not Exits", "UserNotFound"));
    }
    ...
    if (!isPasswordSame) {
      return res
        .status(401)
        .send(ErrorLoader("PassWord Wrong", "UserNotFound"));
    }

    const token = jwt.sign(
      JSON.stringify(findByName),
